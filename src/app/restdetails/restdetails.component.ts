...
export class RestdetailsComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.queryParams.subscribe(data => {
      ...
      if (data.hasOwnProperty('id')) {
        ...
        this.dropdownList = [
          { item_id: 'Restaurant', item_text: 'Restaurant' },
          { item_id: 'Pub', item_text: 'Pub' },
          { item_id: 'Café', item_text: 'Café' },
          { item_id: 'Bistro', item_text: 'Bistro' },
  ...
  importCSVMenu(){
   ...
   csvFileSelected(fileInput){
     if (fileInput.target.files && fileInput.target.files[0]) {
         ...
         reader.onload = function (e : any) {
             ...
             var elem: any = document.getElementById("importcsv");
             elem.value = "";
         }
   ...
   uploadImportCSVItems(cats, subCats, foods){
    var uCats = [];
    var uFoods = [];
    ...
    */
    ...
    foods.forEach(food => {
        ...
        uFoods.push({
            cid: this.db.collection('categories').doc(cid),
            desc: food.desc,
            cover: "",
            id: this.util.makeid(10),
            name: food.name,
